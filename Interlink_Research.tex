...
\title{
    ...
    \small Version 8.0 --- March 2026 \quad \textit{Audit-Candidate Draft}
}
\author{}
\date{}
...
\section*{Abstract}
 ...
 This paper presents InterLink (ILINK), a protocol-level substrate for cross-chain composability powered by recursive zero-knowledge proofs (zk-SNARKs). Using the halo2 proving system, InterLink enables atomic, trustless state transitions across heterogeneous ledgers without requiring honest-majority assumptions for message verification. The architecture combines a decentralized, stake-weighted Relayer Network with a high-throughput Solana Execution Hub responsible for proof verification, global message accounting, and cross-chain coordination.

We formalize the cryptographic primitives, circuit constraints, and incentive design that underpin the protocol. In particular, we show how recursive aggregation yields amortized O(1) on-chain verification costs for batched cross-chain intents while preserving sovereign security properties of connected chains. InterLink is therefore not merely a bridge: it is a verifiable substrate for a Unified Liquidity Layer that supports ``omnichain'' applications whose core logic is decoupled from any single execution environment.

\vspace{0.5in}
Keywords: Zero-Knowledge Proofs, Recursive SNARKs, Atomic Composability, Rust Systems Engineering, Solana Anchor, Liquidity Unification, Hyper-Deflationary Tokenomics, Game-Theoretic Security, Blockchain Interoperability, Decentralized Finance (DeFi).
\end{abstract}
...
\textbf{Circuit constraint structure:}
\begin{enumerate}
  \item \textbf{Participation bitmap} – encode the 512-bit bitfield as advice cells;
        constrain each $b_i \in \{0,1\}$ via $b_i(1-b_i)=0$.
  \item \textbf{Aggregate public key} – running sum over selected $pk_i$:
  ...
  \item \textbf{Pairing gate} – express $e(\sigma, g_2) = e(H(m), \mathsf{apk})$ using
  ...
  \item \textbf{Quorum check} – assert $\sum_i b_i \geq 342$ (i.e.\ $>2/3$ of 512)
...
\textbf{Closed-form cumulative emission:}
$$E_{\text{cum}}(t) = S_0 \sum_{k=0}^{\lfloor t/2 \rfloor} 2 (1-r)^k +
  S_0 (1-r)^{\lfloor t/2\rfloor}(t \bmod 2)$$

\textbf{Circulating supply with burn (Year 1--5 projections):}

\begin{table}[H]
\centering
\begin{tabular}{@{}lrrrrr@{}}
\toprule
\textbf{Scenario} & \textbf{Year 1} & \textbf{Year 2} & \textbf{Year 3} & \textbf{Year 4} & \textbf{Year 5} \\
\midrule
Emissions (M ILINK) & 100.0 & 50.0 & 25.0 & 12.5 & 6.25 \\
\midrule
\textbf{Low volume} (\$1M/day) && & & & \\
\quad Burn (M ILINK) & 1.46 & 1.46 & 0.73 & 0.73 & 0.37 \\
\quad Circulating (M) & 248.5 & 297.1 & 321.4 & 333.2 & 339.1 \\
\midrule
\textbf{Medium volume} (\$10M/day) && & & & \\
\quad Burn (M ILINK) & 14.6 & 14.6 & 7.30 & 7.30 & 3.65 \\
\quad Circulating (M) & 235.4 & 270.8 & 288.5 & 293.7 & 296.3 \\
\midrule
\textbf{High volume} (\$100M/day) && & & & \\
\quad Burn (M ILINK) & 146.0 & 146.0 & 73.0 & 73.0 & 36.5 \\
\quad Circulating (M) & 104.0 & 8.0 & \multicolumn{3}{c}{\textit{net deflationary}} \\
\bottomrule
\end{tabular}
\caption{Projected circulating supply under three volume scenarios (P\,=\,\$1, f\,=\,0.1\%, burn\,=\,40\%).
All figures in millions of ILINK tokens; cumulative includes team/investor unlock schedules.}
\label{tab:supply-schedule}
\end{table}

\noindent\textbf{Monte Carlo summary (10,000 simulations):}
At median adoption (\$10M daily volume, $\sigma=50\%$ log-normal volume shocks),
the protocol is expected to become net-deflationary in \textbf{Year 3} with 95\% confidence
interval $[2.4,\,4.1]$ years. At high adoption (\$100M/day), deflationary crossover occurs
within \textbf{12--18 months} of mainnet launch.
...
\centering
\begin{tabular}{l|c|c}
 & \textbf{Honest Relayer} & \textbf{Byzantine Relayer} \\ \hline
\textbf{Protocol Accepts} &
  $R$: $+50 - 5 = +45$ ILINK &
  $R$: $P_{\text{verif}} \cdot (-100{,}000) = -100{,}000$ ILINK \\
  & $P$: $+\text{volume}$ &
  $P$: $-G = -500{,}000$ ILINK \\ \hline
\textbf{Protocol Rejects} &
  $R$: $-5$ ILINK (gas wasted) &
  $R$: $-50{,}000$ ILINK (50\% slash) \\
  & $P$: $-\text{liveness}$ &
  $P$: $+\text{security}$ \\ \hline
\end{tabular}
\caption{Relayer--Protocol payoff matrix with concrete numerical values.
Payoffs in ILINK tokens; Protocol payoffs are qualitative except where quantified.}
\label{tab:payoff}
\end{table}
...
\textbf{Attack-profitability threshold:} For Byzantine behavior to be rational,
...
\begin{itemize}
    \item \textbf{Honest Equilibrium:} Relayers earn $+45$ ILINK per message and accumulate reputation
    \item \textbf{Malicious Deterrence:} Expected loss of $\approx 100{,}000$ ILINK per attempt
    \item \textbf{Nash Equilibrium:} Honest strategy strictly dominates for all stake levels $S \geq 100$ ILINK
\end{itemize}
...
\begin{tabular}{@{}lllll@{}}
\toprule
\textbf{Milestone} & \textbf{Target} & \textbf{Status} & \textbf{Auditor} & \textbf{Notes} \\
\midrule
Halo2 circuit v1 (Merkle + BLS) & Q2 2026 & In progress & Pending & See \cref{sec:bls-circuit} \\
Devnet (Eth $\to$ Solana) & Q3 2026 & Planned & --- & Internal testnet \\
Security audit (circuits) & Q3 2026 & Planned & TBD & OtterSec / Trail of Bits \\
Public testnet & Q4 2026 & Planned & --- & Open relayer set \\
Token launch (ILINK) & Q4 2026 & Planned & --- & Audit gate required \\
Mainnet v1 (Eth + Solana) & Q1 2027 & Planned & --- & Post-audit \\
Multi-chain (Cosmos, Arbitrum) & Q2--Q3 2027 & Planned & --- & IBC + L2 gateways \\
Full recursive aggregation & Q4 2027 & R\&D & --- & Folding scheme finalized \\
Developer SDK \& documentation & Q1 2028 & Planned & --- & Public npm + crates.io \\
\bottomrule
\end{tabular}
\caption{InterLink current development status and forward roadmap (as of March 2026).}
\label{tab:roadmap}
\end{table}

\subsubsection{Implementation Timeline}

\textbf{Phase 1 (Q2--Q3 2026): Foundation \& Devnet}
\begin{itemize}
...
\textbf{Phase 2 (Q4 2026 -- Q1 2027): Public Testnet \& Mainnet}
\begin{itemize}
...
\textbf{Phase 3 (Q2--Q4 2027): Ecosystem Expansion}
\begin{itemize}
...
\textbf{Phase 4 (2028+): Mass Adoption \& Advanced Features}
\begin{itemize}
...
\appendix
\section{Background on Blockchain Technology}

\subsection{Background on Blockchain Technology (Summary)}
\label{sec:blockchain-bg}

This appendix provides a condensed reference for readers new to blockchain fundamentals.
For a comprehensive treatment, see Narayanan et al.\ \cite{bitcoin-book}.
...
\bibitem{ronin-hack}
Sky Mavis Security Team. \textit{Ronin Network Post-Mortem: \$625M Exploit Analysis}.
March 2022. \url{https://roninblockchain.substack.com/p/community-alert-ronin-validators}

\bibitem{wormhole-hack}
Certora / Jump Crypto. \textit{Wormhole Bridge Exploit Post-Mortem: \$320M Infinite Mint Vulnerability}.
February 2022. \url{https://extropy-io.medium.com/solanas-wormhole-hack-post-mortem-analysis-3b68b9e88e13}

\bibitem{bitcoin-book}
Narayanan, A., Bonneau, J., Felten, E., Miller, A., Goldfeder, S.
\textit{Bitcoin and Cryptocurrency Technologies}. Princeton University Press, 2016.
...
\hrule
\bigskip
\noindent\textbf{Get involved:}
\begin{itemize}
  \item \textbf{GitHub:} \url{https://github.com/MeridianAlgo/Cobalt}
  \item \textbf{We are hiring} Rust + zk engineers and circuit auditors.
  \item \textbf{Audit timeline:} Q3 2026 (OtterSec / Trail of Bits engagement in progress).
\end{itemize}
